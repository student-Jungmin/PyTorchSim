...
| Qwen2-VL / Qwen2.5-VL | 🤗 | ⚠️ | `tests/models/Qwen/test_qwen_vl.py` — end to end at reduced width, and at the **real 1280 vision width** with `--preset 2b --eager-splice`. M-RoPE, the 3D patchifier and the image splice are all exercised. Needs the i64 `get_structured_state` fix in triton_shared and the replicate-axes fix in triton-npu; `--eager-splice` trades the splice's kernel for an eager fallback, which is what the sized presets need |
