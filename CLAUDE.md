...
- **Parallel runs sharing a `TORCHSIM_DUMP_PATH`** share every per-kernel workdir, and the artifacts in it are fixed names. Three locks in the workdir keep that honest: `.compile.lock` (tnpu compile), `.launch.lock` (Spike, `runtime/*.raw`) and `.timing.lock` (`trace.so`, `trace_shape.txt`, the tile graph, gem5's `m5out`). They are taken one at a time, never nested. Give each experiment its own dump path anyway — the locks make sharing correct, not fast.
