...
**32개 연산 / 612개 케이스.** `ops/*.py`에서 생성한다 (`python timing_mode_validation/index.py`) — 케이스를 고치면 이 파일도 다시 만들어진다.
...
## `gemm` — 454개 (모델 454 / 추가 0)

A @ B, the projection shape every transformer layer is made of.

| 파라미터 | 범위 | 모델 값 | 추가 값 |
|---|---|---|---|
| `M` | 1 – 98304 | 1, 32, 128, 256, 512, 1024, 2048, 3584, 4096, 8192, 16384, 32768, 65536, 98304 | — |
| `K` | 384 – 28672 | 384, 768, 1024, 1536, 2048, 2560, 3072, 4096, 5120, 8192, 10240, 14336, 25600, 28672 | — |
| `N` | 384 – 262208 | 384, 768, 1024, 1152, 1536, 2048, 2304, 2560, 3072, 4096, 5120, … , 128256, 151936, 262208 | — |
